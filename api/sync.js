...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (lastBlockResult.rows[0].last_block) {
      ...
      const startTimestamp = Math.floor(new Date('2025-10-29T00:00:00+09:00').getTime() / 1000);
      const blocksDiff = latestBlockData.timestamp - startTimestamp;
      fromBlock = Math.max(0, latestBlock - blocksDiff);
    ...
    // 최대 50,000 블록까지만 (약 14시간치)
    const toBlock = Math.min(fromBlock + 50000, latestBlock);
